...
def train(args):
    # Load the dataset
    trn_df = parse_data('squad', 'train', mode='train')#in_file=f'../../data/{args.dataset}/trn.tsv')
    val_df = parse_data('squad', 'validation', mode='validation')#in_file=f'../../data/{args.dataset}/val.tsv')
...
def test(args):
    te_df = parse_data(in_file='squad', split='train', mode='test') #f'../../data/{args.dataset}/tst.tsv')
    print('Data loaded!!!')
    ...
    if args.timestamp == '0':
       ...
       #tokenizer = T5TokenizerFast.from_pretrained('/home/ishikaa2/CS598BAN-proj/models/t5/t5-base_squad_cvae_2023-04-03-20-11-05/checkpoint-10000')
    ...
    if args.timestamp == '0':
       ...
       #model = Seq2SeqModel.from_pretrained('/home/ishikaa2/CS598BAN-proj/models/t5/t5-base_squad_cvae_2023-04-03-20-11-05/checkpoint-10000')
...
#    print(f"Test ppl: {ppl}")
    ...
    for i in range(len(pred_list)):
        sentences = pred_list[i]['generated'].split('\t')
        scores.append(SelfBLEU(sentences).get_score())
        bleu_scores.append(BLEU(sentences).get_score())'''
...
if __name__ == '__main__':
    ...
    p.add_argument('-t', '--task', type=str, default="test", 
                   help="specify the task to do: (train)ing, ft(finetune), (eval)uation")
    p.add_argument('-c', '--ckpt', type=str, default="18000", 
                   help="Model checkpoint")
    p.add_argument('-time', '--timestamp', type=str, default='2023-04-04-23-42-02',
                   help="Model checkpoint")
